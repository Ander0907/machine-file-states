...
async function runDemos() {
  ...
  try {
    const result2 = await fileJob.processFileWithRetry(
      'file-retry-001',
      {
        filename: 'data.csv',
        size: 1024,
  ...
  try {
    const result3 = await fileJob.processFileWithRetry(
      'file-max-retry-001',
      {
      ...
      3
    ); // Fail 3 times, should be REJECTED
  ...
  try {
    const result5 = await fileJob.processFileWithRejection(
      'file-duplicate-001',
      {
